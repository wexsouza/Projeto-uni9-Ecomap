document.addEventListener("DOMContentLoaded", function () {
  ...
  var textos = {
    ...
    coletor: {
      ...
      link: "Criar conta de coletor",
      href: "cadastro-coletor.html",
    },
  };
  function preencherLembrado() {
    var salvo;
    try {
      salvo = JSON.parse(localStorage.getItem(DB_KEYS.lembrar));
    } catch (e) {
      salvo = null;
    }
    identificador.value =
      salvo && salvo.tipo === tipoAtual ? salvo.identificador : "";
    lembrar.checked = !!(salvo && salvo.tipo === tipoAtual);
  }
  function selecionarTipo(tipo) {
    tipoAtual = tipo;
    var atual = textos[tipo];
    titulo.textContent = atual.titulo;
    texto.textContent = atual.texto;
    label.textContent = atual.label;
    identificador.placeholder = atual.placeholder;
    ...
    tabDoador.setAttribute("aria-selected", String(tipo === "doador"));
    tabColetor.setAttribute("aria-selected", String(tipo === "coletor"));
    limparErros(form);
    esconderAlerta();
    preencherLembrado();
  }
  tabDoador.addEventListener("click", function () {
    selecionarTipo("doador");
  });
  tabColetor.addEventListener("click", function () {
    selecionarTipo("coletor");
  });
  preencherLembrado();
  ...
  if (msg) {
    ...
    var msgTipo = sessionStorage.getItem("ecomaps_msg_tipo");
    if (msgTipo) selecionarTipo(msgTipo);
  ...
  form.addEventListener("submit", function (event) {
    ...
    if (!id)
      mostrarErro(
        "identificador",
        "erroIdentificador",
        "Informe seu " + textos[tipoAtual].label.toLowerCase() + ".",
      );
    if (!pass) mostrarErro("senha", "erroSenha", "Informe sua senha.");
    if (!id || !pass) return;
    var chave = tipoAtual === "coletor" ? DB_KEYS.coletores : DB_KEYS.doadores;
    var busca = id.toLowerCase();
    var numeros = somenteNumeros(id);
    var usuario = dbLer(chave).find(function (u) {
      return (
        (u.email || "").toLowerCase() === busca ||
        (numeros && somenteNumeros(u.telefone || "") === numeros) ||
        (numeros && somenteNumeros(u.cpf || "") === numeros)
      );
    });
    ...
    if (lembrar.checked)
      localStorage.setItem(
        DB_KEYS.lembrar,
        JSON.stringify({ tipo: tipoAtual, identificador: id }),
      );
    else localStorage.removeItem(DB_KEYS.lembrar);
    window.location.href =
      tipoAtual === "coletor"
        ? "dashboard-coletor.html"
